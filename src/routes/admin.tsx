...
export const Route = createFileRoute("/admin")({
  beforeLoad: ({ location }) => {
    // Allow /admin/login publicly; protect everything else.
      ...
      <div className="mx-auto flex min-h-screen max-w-[1400px]">
        ...
        <aside className="hidden w-64 shrink-0 border-r border-white/10 bg-sidebar/60 backdrop-blur-xl md:flex md:flex-col">
          <Link to="/admin" className="flex items-center gap-2 px-5 py-5">
            ...
            <div>
              <div className="text-sm font-semibold">CAB Admin</div>
          ...
          <nav className="flex-1 space-y-1 px-3">
            {navItems.map((n) => (
              <Link
                ...
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-white/5 hover:text-foreground"
                activeProps={{
                  className:
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <header className="sticky top-0 z-30 flex items-center justify-between gap-2 border-b border-white/10 bg-background/70 px-4 py-3 backdrop-blur md:hidden">
            <Link to="/admin" className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold">CAB Admin</span>
            </Link>
          ...
          <nav className="flex gap-1 overflow-x-auto border-b border-white/10 bg-background/40 px-3 py-2 md:hidden">
            {navItems.map((n) => (
              <Link
                ...
                to={n.to}
                activeOptions={{ exact: n.exact }}
                className="shrink-0 rounded-lg px-3 py-2 text-xs text-muted-foreground"
                activeProps={{
